...
def cleanpulllist(list):
    '''Removes Backports and non-merged/closed PRs from list of pull requests'''
    for x in list:
      if not x.merged :
          list.remove(x)
    return list
...
def main():
    ...
    try:
      ...
      pl=docsrepo.get_pulls(state='closed',base='master')
      ...
      raise
    # pull_list=cleanpulllist(pull_list[:20]) // (may not be needed)
    ...
    pr_base  = target
    pr_body  = ("This pull request has been generated by the canonical-doc-utilities "
                "backport command.\n\n"
                "It has successfully cherry-picked individual commits from a different "
                "branch of this repository, which should merge without issue. It is "
                "advisable to check the changes only occur where you expect them!\n\n"
                "The original PR this was ported from can be viewed here:" + pull.html_url )
    # raise a PR
    try:
    ...
    #   - fetch PR as issue
    issue = docsrepo.get_issue(pr.number)
    # - check label exists
    try:
